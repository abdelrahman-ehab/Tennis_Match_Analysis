...
class MiniCourt():
    ...
    def set_court_lines(self):
        # A function to draw lines between the points
        self.lines = [
            (0, 2),
            (4, 5),
            (6,7),
    ...
    # Drawing the court on the white rectangle
    def draw_court(self,frame):
       # Drawing the red dots 
        ...
        for line in self.lines:
            # 0th pos -> 0,1
            # 1st pos -> 2,3  --> that's why we multiply by 2
    ...
    def draw_background_rectangle(self,frame):
        shapes = np.zeros_like(frame,np.uint8) # dummy image all zeroes
        # Draw the rectangle
        cv2.rectangle(shapes, (self.start_x, self.start_y), (self.end_x, self.end_y), (255, 255, 255), cv2.FILLED)
        out = frame.copy()
        alpha=0.5 # Transparency value
        mask = shapes.astype(bool)
        # Blend the original frame and the white rectangle
        out[mask] = cv2.addWeighted(frame, alpha, shapes, 1 - alpha, 0)[mask]

        return out

    def draw_mini_court(self,frames):
        # apply the rectangle on all the frames
        output_frames = []
        for frame in frames:
    ...
    # This function converts raw pixel coordinates from video frames to normalized mini-court coordinates
